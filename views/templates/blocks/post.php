...
<div id="<?php echo esc_attr( $blockId ); ?>" class="<?php echo implode( ' ', ! empty( $outerwrap ) ? $outerwrap : [] ); ?>"<?php echo sprintf( '%s', $animation ? ' data-gutengeek-animation="'.$animation.'"' : '' ) ?>>
				...
				<div class="gutengeek-post-container">
					<div class="gutengeek-post-inner">
						...
						if ( ! empty($attributes['enableFilter']) && $attributes['enableFilter'] ) {
							gtg_get_template( 'blocks/post/filter.php', [ 'attributes' => $attributes ] );
						}
						?>
						...
						<div class="gutengeek-post__container-wrap">
							<?php foreach ( $query as $index => $query_item ) : ?>
								...
								<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $container_classes ) ) ); ?>">
									<div class="<?php echo esc_attr( implode( ' ', ! empty( $wrap ) ? $wrap : [] ) ); ?>" <?php echo sprintf( '%s', $attrs_string ) ?>>
										...
										<?php if ( isset( $query_item['query'] ) && $query_item['query'] ) :
											...
											while ( $post_query->have_posts() ) :
												...
												<?php gtg_get_template( 'blocks/loop-post-grid.php', [ 'attributes' => $attributes, 'index' => $index ] ); ?>
									 ...
									 * Navigation.
									 */
									if ( ! empty($attributes['enablePagination']) && $attributes['enablePagination'] ) {
										gtg_get_template( 'blocks/post/navigation.php', [ 'attributes' => $attributes, 'query' => $post_query ] );
									}
									?>
