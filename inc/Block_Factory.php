...
use GutenGeek\Abstracts\Block as AbstractBlock;
use GutenGeek\Blocks\Gravity_Form;
use GutenGeek\Blocks\Mailchimp;
use GutenGeek\Blocks\Post_Carousel;
use GutenGeek\Blocks\Post_Grid;
use GutenGeek\Blocks\Post_Masonry;
use GutenGeek\Blocks\Post_Timeline;
use GutenGeek\Blocks\Cf7;
...
class Block_Factory {
	...
	public function register_block_types() {
		...
		foreach ( $this->get_blocks() as $slug => $block ) {
			...
			if ( !gutengeek_get_option( '_gutengeekblock_bundle_assets' ) && isset( $attributes['editor_script'] )
	...
	public function custom_form_validated( $actions = [], $params = [] ) {
		...
		try {
			$email_from = ! empty( $params['email-from'] ) ? base64_decode( sanitize_text_field( $params['email-from'] ) ) : '';
			$email_reply = ! empty( $params['email-reply'] ) ? base64_decode( sanitize_text_field( $params['email-reply'] ) ) : '';
			$email_subject = ! empty( $params['email-subject'] ) ? base64_decode( sanitize_text_field( $params['email-subject'] ) ) : '';
			$email_recipients = ! empty( $params['email-recipients'] ) ? base64_decode( $params['email-recipients'] ) : '';
			$email_cc = ! empty( $params['email-cc'] ) ? base64_decode( sanitize_text_field( $params['email-cc'] ) ) : '';
			$email_bcc = ! empty( $params['email-bcc'] ) ? base64_decode( sanitize_text_field( $params['email-bcc'] ) ) : '';
			$email_body = ! empty( $params['email-body'] ) ? base64_decode( sanitize_text_field( $params['email-body'] ) ) : '';
