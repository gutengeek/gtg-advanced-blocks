...
use GutenGeek\Plugin as Plugin;
 ...
 */
if ( ! function_exists( 'gutengeek_is_gutenberg_installed' ) ) {
	 ...
	 *
	 */
	function gutengeek_is_gutenberg_installed( $plugin_path ) {
		$plugins = get_plugins();
...
if ( ! function_exists( 'gutengeek_block_factory' ) ) {

	function gutengeek_block_factory() {
		return Plugin::instance()->block_factory;
	}

}

if ( ! function_exists( 'gutengeek_load_fontawesome_icons' ) ) {
	/**
	 * Get Json Data.
	 *
	 * @return Array
	 * @since 1.0.0
	 */
	function gutengeek_load_fontawesome_icons() {
		...
		$fontawesome_file = GUTENGEEK_DIR . 'assets/libs/fontawesome-icons.json';
...
if ( ! function_exists( 'gutengeek_render_svg_html' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function gutengeek_render_svg_html( $icon ) {
		...
		if ( is_string($icon) ) {
			...
			$lib = $icon['lib'];
			if ($lib === 'fontawesome') {
				$json = gutengeek_load_fontawesome_icons();
...
if ( ! function_exists( 'gutengeek_get_image_sizes' ) ) {
	 ...
	 */
	function gutengeek_get_image_sizes() {
...
if ( ! function_exists( 'gutengeek_get_post_types' ) ) {
	 ...
	 * @access public
	 */
	function gutengeek_get_post_types() {
...
if ( ! function_exists( 'gutengeek_get_related_taxonomy' ) ) {
	 ...
	 * @access public
	 */
	function gutengeek_get_related_taxonomy() {
...
if ( ! function_exists( 'gutengeek_parse_blocks' ) ) {
	 ...
	 * @return array
	 */
	function gutengeek_parse_blocks( $content ) {
		global $wp_version;
...
if ( ! function_exists( 'gutengeek_build_query' ) ) {
	 ...
	 * @return WP_Query
	 */
	function gutengeek_build_query( $attributes, $block_type ) {
		global $wp_query;
...
if ( ! function_exists( 'gutengeek_get_option' ) ) {

	function gutengeek_get_option( $key, $default = false, $network_override = false ) {
		// is network
...
if ( ! function_exists( 'gutengeek_update_option' ) ) {

	function gutengeek_update_option( $key, $value, $network = false ) {
 ...
 */
function gutengeek_timeline_get_classes( $attributes ) {
 ...
 */
function gutengeek_timeline_get_align_classes( $attributes, $index_val ) {
 ...
 * @return string            output HTML/String.
 */
function gutengeek_timeline_get_day_align_classes( $attributes, $index_val ) {
...
if ( ! function_exists( 'gutengeek_get_static_css_file_path' ) ) {
	/**
	 ...
	 * @return mixed|void
	 */
	function gutengeek_get_static_css_file_path( $post_id = null ) {
		$filename = GUTENGEEK_STATIC_CSS_PATH . $post_id . '.css';

		return apply_filters( 'gutengeek_post_css_file_path', $filename, $post_id );
	}
}

if ( ! function_exists( 'gutengeek_get_static_css_file_url' ) ) {
	/**
	 ...
	 * @return mixed|void
	 */
	function gutengeek_get_static_css_file_url( $post_id = null ) {
		$file = gutengeek_get_static_css_file_path( $post_id );
		$url  = '';
		if ( file_exists( $file ) ) {
			$url = GUTENGEEK_STATIC_CSS_URL . $post_id . '.css';
		}
...
if ( ! function_exists( 'gutengeek_shape_backgrounds' ) ) {
	function gutengeek_shape_backgrounds() {
...
if ( ! function_exists( 'gutengeek_render_shape_background' ) ) {

	/**
	 * print shape divider
	 */
	function gutengeek_render_shape_background( $name = '' ) {
		static $shapes;
		if ( empty( $shapes ) ) {
...
if ( ! function_exists( 'gutengeek_divider_styles' ) ) {
	function gutengeek_divider_styles() {
...
if ( ! function_exists( 'gutengeek_get_color_presets' ) ) {

	/**
	 * get color presets
	 */
	function gutengeek_get_color_presets() {
...
if ( ! function_exists( 'gutengeek_get_button_styles' ) ) {

	/**
	 * get color presets
	 */
	function gutengeek_get_button_styles() {
...
if ( ! function_exists( 'gutengeek_global_setting_name' ) ) {

	function gutengeek_global_setting_name( $name = '', $default = false ) {
		$settings = gutengeek()->global_settings ? gutengeek()->global_settings : [];
...
if ( ! function_exists( 'gutengeek_setup_sse_event' ) )  {

	function gutengeek_setup_sse_event() {
...
if ( ! function_exists( 'gutengeek_emit_sse_event' ) ) {

	/**
	 * sse emit message
	 */
	function gutengeek_emit_sse_event( $data = [] ) {
		if ( ! defined( 'WP_CLI' ) ) {
			echo "event: message\n";
...
if ( ! function_exists( 'gutengeek_get_saved_items' ) ) {

	/**
	 * get all saved blocks items
	 */
	function gutengeek_get_saved_items() {
		$args = [
			'post_type' => 'wp_block',
			'post_status' => 'publish',
			'posts_per_page' => -1,
...
if ( ! function_exists( 'gutengeek_is_remote_item_saved' ) ) {
	 ...
	 * @return bool
	 */
	function gutengeek_is_remote_item_saved( $tmp_id = false ) {
		$args = [
			'post_type' => 'wp_block',
			'post_status' => 'publish',
			'posts_per_page' => -1,
		...
		return apply_filters( 'gutengeek_is_remote_item_saved', count( $posts ) > 0 );
	}

}

if ( ! function_exists( 'gutengeek_get_custom_fonts' ) ) {
	 ...
	 * @return array
	 */
	function gutengeek_get_custom_fonts() {
		global $wpdb;
		$font_ids = $wpdb->get_col( $wpdb->prepare(
			"SELECT font_id FROM {$wpdb->prefix}gutengeek_block_fonts WHERE font_status = %s", 'publish'
		) );

		$font_data = [];
		foreach ($font_ids as $id) {
			$font_data[] = gutengeek_get_font_data($id);
		}
...
if ( ! function_exists( 'gutengeek_get_font_data' ) ) {
	 ...
	 */
	function gutengeek_get_font_data( $font_id = false ) {
		if ( ! $font_id ) {
			return $font_data = [];
		} else {
			global $wpdb;
			$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}gutengeek_block_fonts WHERE font_id = %d", $font_id );
			$font_data = $wpdb->get_row( $query, ARRAY_A );
			if ( ! $font_data ) {
				return new WP_Error( __( 'Font not found.' ) );
			}

			$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}gutengeek_block_font_items WHERE font_id = %d", $font_id );
			...
			if ( $font_items ) {
				...
				foreach ( $font_items as $k => $font_item ) {
					...
					// each font files
					$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}gutengeek_block_font_item_attachments WHERE font_item_id = %d", $font_item_id );
...
if ( ! function_exists( 'gutengeek_get_custom_icons' ) ) {
	function gutengeek_get_custom_icons() {
		return get_option( GutenGeek\Modules\CustomIcons::OPTION_NAME, [] );
	}
}

if ( ! function_exists( 'gutengeek_get_icon_data' ) ) {
	 ...
	 * @return mixed | array | WP_Error
	 */
	function gutengeek_get_icon_data( $icon_id = false ) {
		if ( ! $icon_id ) {
			return [];
		} else {
			global $wpdb;
			$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}gutengeek_block_icons WHERE icon_id = %d", $icon_id );
...
if ( ! function_exists( 'gutengeek_font_name_exists' ) ) {
	/**
	 * check font exists by name
	 *
	 * @return int if its already exists, false otherwise
	 */
	function gutengeek_font_name_exists( $font_name = '' ) {
		global $wpdb;
		$query = $wpdb->prepare( "SELECT {$wpdb->prefix}gutengeek_block_fonts WHERE font_name = %s", $font_name );
		return apply_filters( 'gutengeek_font_name_exists', $wpdb->get_var($query) );
	}
}

add_filter( 'upload_mimes', 'gutengeek_upload_addition_mimes', 1, 1 );
if ( ! function_exists( 'gutengeek_upload_addition_mimes' ) ) {
	function gutengeek_upload_addition_mimes( $mime_types ) {
...
if ( ! function_exists( 'gutengeek_get_setting' ) ) {

	/**
	 * get global settings
	 */
	function gutengeek_get_setting( $name = '', $default = '') {
		$settings = gutengeek()->get_settings();
		$value = ! empty( $settings[ $name ] ) ? $settings[ $name ] : $default;
		return apply_filters( 'gutengeek_get_setting', $value );
	}

}

if ( ! function_exists( 'gutengeek_get_sm_breakpoint_setting' ) ) {

	/**
	 * get sm breakpoints settings
	 */
	function gutengeek_get_sm_breakpoint_setting() {
		return apply_filters( 'gutengeek_get_sm_breakpoint_setting', gutengeek_get_setting( 'sm_breakpoint', GUTENGEEK_SM_BREAKPOINT ) );
	}

}

if ( ! function_exists( 'gutengeek_get_xs_breakpoint_setting' ) ) {

	/**
	 * get xs breakpoints settings
	 */
	function gutengeek_get_xs_breakpoint_setting() {
		return apply_filters( 'gutengeek_get_xs_breakpoint_setting', gutengeek_get_setting( 'sm_breakpoint', GUTENGEEK_XS_BREAKPOINT ) );
	}

}

if ( ! function_exists( 'gutengeek_masks_presets' ) ) {
	/**
	 * define mask presets
	 */
	function gutengeek_masks_presets() {
		return apply_filters( 'gutengeek_masks_presets', [
	 		[
	 			'name' => __('Mask 01', 'gutengeek'),
	 			'slug' => 'mask-01',
	 			'url' => GUTENGEEK_URL . 'assets/images/masks/mask-01.svg',
	 			'string' => ''
	 		],
	 		[
	 			'name' => __('Mask 02', 'gutengeek'),
	 			'slug' => 'mask-02',
	 			'url' => GUTENGEEK_URL . 'assets/images/masks/mask-02.svg',
	 			'string' => ''
	 		],
	 		[
	 			'name' => __('Mask 03', 'gutengeek'),
	 			'slug' => 'mask-03',
	 			'url' => GUTENGEEK_URL . 'assets/images/masks/mask-03.svg',
	 			'string' => ''
	 		],
	 		[
	 			'name' => __('Mask 04', 'gutengeek'),
	 			'slug' => 'mask-04',
	 			'url' => GUTENGEEK_URL . 'assets/images/masks/mask-04.svg',
	 			'string' => ''
	 		]
	 	] );
	}
