...
use GutenGeek\Tables\CustomIconsTable as Table;
...
class CustomIcons {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_media();
		wp_enqueue_script( 'gutengeek-custom-icons', GUTENGEEK_URL . 'assets/js/admin/custom-icons.js', [ 'jquery', 'wp-util', 'svg-painter', 'wp-element', 'wp-i18n', 'gutengeek-admin-script' ],
			GUTENGEEK_VER );
		wp_localize_script( 'gutengeek-custom-icons', 'gutengeek_custom_icon_data', [
			'nonce'        => wp_create_nonce( 'gutengeek-save-icon-nonce' ),
			'icon'         => gutengeek_get_icon_data( $iconId ),
			'isEdit'       => $iconId ? 1 : 0,
	...
	public function upload_custom_icon() {
		...
		try {
			...
			$icon_name = ! empty( $_REQUEST['icon_name'] ) ? $_REQUEST['icon_name'] : $upload_handler['name'];
			$request = $_REQUEST;
			$request = array_merge( [
				'icon_name'     => $icon_name,
			...
			$response = [
				'icon'    => gutengeek_get_icon_data( $icon_id ),
	...
	public function update_custom_icon() {
		...
		try {
			$icon_name = ! empty( $_REQUEST['icon_name'] ) ? $_REQUEST['icon_name'] : '';
	...
	private function is_valid_icon_name( $icon_id, $icon_name = '' ) {
		...
		return apply_filters( 'gutengeek_is_icon_name_valid', ! $wpdb->get_var( $query ), $icon_id, $icon_name );
	}
	...
	public function bulk_actions() {
		...
		global $wpdb;
		$icon_ids = ! empty( $_REQUEST['icons'] ) ? $_REQUEST['icons'] : [];
		switch ( $action ) {
			case 'trash':
				$count = 0;
	...
	public static function get_supported_icon_sets() {
		$icon_sets = [
			'icomoon'   => 'GutenGeek\Modules\Icon_Sets\Icomoon',
			'fontastic' => 'GutenGeek\Modules\Icon_Sets\Fontastic',
		];
	...
	public static function get_upload_folder_path() {
		 ...
		 * Upload file path.
		 *
		 * Filters the path to a file uploaded using GutenGeek forms.
		 *
