<?php
namespace GutenGeek\Modules;
use GutenGeek\Tables\CustomFontsTable as Table;
...
class CustomFonts {
	...
	public function enqueue_scripts() {
		...
		$isCreate = isset( $_REQUEST['add_new'] ) ? $_REQUEST['add_new'] : false;
		if ( $pagenow !== 'admin.php' || $page !== 'gutengeek-custom-fonts' ) {// || (! $fontId && $isCreate === false)
			return;
		}
		// enqueue scripts for custom fonts page
		wp_enqueue_media();
		wp_enqueue_script( 'gutengeek-custom-fonts', GUTENGEEK_URL . 'assets/js/admin/custom-fonts.js', [ 'jquery', 'wp-util', 'svg-painter', 'wp-element', 'wp-i18n', 'gutengeek-admin-script' ],
			GUTENGEEK_VER );
		wp_localize_script( 'gutengeek-custom-fonts', 'gutengeek_custom_font_data', [
			...
			'font' => gutengeek_get_font_data( $fontId ),
		]);
	}
	...
	public function process_save($request = []) {
		...
		try {
			...
			if ( ! wp_verify_nonce($nonce, 'gutengeek-save-font-nonce') ) {
				return new \WP_Error( 'gutengeek_none_invalid', __( 'Invalid nonce', 'gutengeek' ) );
			}
	...
	public function process_save_font($font_data) {
		...
		if ( ! $font_id ) {
			$inserted = $wpdb->insert( $wpdb->prefix . 'gutengeek_block_fonts', [
			...
			$updated = $wpdb->update(
				$wpdb->prefix . 'gutengeek_block_fonts',
				[
					'font_name' => $font_data['font_name'],
	...
	private function is_valid_font_name( $font_id, $font_name = '' ) {
		global $wpdb;
		$query = $wpdb->prepare( "SELECT font_id FROM {$wpdb->prefix}gutengeek_block_fonts WHERE 1 = 1 AND font_name = %s", $font_name );
		if ( $font_id ) {
			$query .= $wpdb->prepare( " AND font_id != %d", $font_id );
		}

		return apply_filters( 'gutengeek_is_font_name_valid', ! $wpdb->get_var( $query ), $font_id, $font_name );
	}
	...
	private function process_save_font_item( $font_id, $item = [] ) {
		...
		if ( ! $font_item_id ) {
			$inserted = $wpdb->insert(
				$wpdb->prefix . 'gutengeek_block_font_items',
				[
	...
	private function process_save_font_file( $font_item_id, $file ) {
		...
		if ( ! $font_item_attach_id ) {
			$inserted = $wpdb->insert(
				$wpdb->prefix . 'gutengeek_block_font_item_attachments',
				[
	...
	public function single_item_action() {
		...
		// get font data
		$font = gutengeek_get_font_data( $font_id );
	...
	public function trash_item_action( $action, $font_id, $font ) {
		global $wpdb;
		// update status
		$wpdb->update( $wpdb->prefix . 'gutengeek_block_fonts', [ 'font_status' => 'trash' ], [ 'font_id' => $font_id ] );
	...
	public function restore_item_action( $action, $font_id, $font ) {
		global $wpdb;
		// update status
		$wpdb->update( $wpdb->prefix . 'gutengeek_block_fonts', [ 'font_status' => 'publish' ], [ 'font_id' => $font_id ] );
	...
	public function delete_item_action( $action, $font_id, $font ) {
		global $wpdb;
		// update status
		$wpdb->delete( $wpdb->prefix . 'gutengeek_block_fonts', [ 'font_id' => $font_id ] );
	...
	public function save_custom_font() {
		...
		try {
			...
			$font = ! empty( $_REQUEST['font'] ) ? $_REQUEST['font'] : [];
