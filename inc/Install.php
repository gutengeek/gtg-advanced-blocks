...
class Install {
	...
	public static function init_options() {
		...
		if ( ! get_option( '_gutengeek_global_settings' ) ) {
			add_option( '_gutengeek_global_settings', apply_filters( 'gutengeek_init_default_global_settings', [
				'css_method' => 'file'
			] ) );
		}
	}
	...
	public static function create_folders() {
		...
		if ( !$wp_filesystem->is_dir( GUTENGEEK_STATIC_CSS_PATH ) || !$wp_filesystem->exists( GUTENGEEK_STATIC_CSS_PATH ) ) {
			$wp_filesystem->mkdir( GUTENGEEK_STATIC_CSS_PATH );
		}
	}
	...
	public static function get_schema() {
		...
		$tables = "
CREATE TABLE {$wpdb->prefix}gutengeek_block_fonts (
...
CREATE TABLE {$wpdb->prefix}gutengeek_block_font_items (
...
CREATE TABLE {$wpdb->prefix}gutengeek_block_font_item_attachments (
...
CREATE TABLE {$wpdb->prefix}gutengeek_block_icons (
