...
use GutenGeek\Abstracts\Post_Block;
...
class Post_Grid extends Post_Block implements InterfaceBlock {
	...
	public function render_callback( $attributes ) {
		...
		if ( ! empty($attributes['enableFilter']) && $attributes['enableFilter'] && ! $attributes['filterAjax'] && $attributes['termType'] ) {
			foreach ( $attributes['termType'] as $term ) {
				...
				$new_query['query']       = gutengeek_build_query( $attributes, 'grid' );
				$new_query['term']        = $term;
				$query[]                  = $new_query;
			}
		...
		$this->gutengeek_get_post_html( $attributes, $query, 'grid' );
	...
	public function ajax_loadmore() {
		...
		$attributes['categories'] = isset( $_POST['term'] ) && $_POST['term'] ? sanitize_text_field( $_POST['term'] ) : '';

		$query = gutengeek_build_query( $attributes, 'grid' );

		if ( $query->have_posts() ) {
			$return['status'] = true;
			ob_start();
			while ( $query->have_posts() ) : ( $query->the_post() );
				gutengeek_get_template( 'blocks/loop-post-grid.php', [ 'attributes' => $attributes ] );
			endwhile;
