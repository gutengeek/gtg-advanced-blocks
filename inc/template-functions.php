...
if ( ! function_exists( 'gutengeek_block_add_body_class' ) ) {
	function gutengeek_block_add_body_class($classes) {
 ...
 */
function gutengeek_get_template_part( $slug, $name = '' ) {
	...
	if ( !$template ) {
		if ( $name ) {
			$template = locate_template(
				[
					"{$slug}-{$name}.php",
					gutengeek()->template_path() . "{$slug}-{$name}.php",
				]
			);

			if ( !$template ) {
				$fallback = gutengeek()->plugin_path() . "/views/templates/{$slug}-{$name}.php";
		...
		if ( !$template ) {
			...
			$template = locate_template(
				[
					"{$slug}.php",
					gutengeek()->template_path() . "{$slug}.php",
				]
			);
		}
 ...
 */
function gutengeek_get_template( $template_name, $args = [], $template_path = '', $default_path = '' ) {
	$cache_key = sanitize_key( implode( '-', [ 'template', $template_name, $template_path, $default_path ] ) );
	$template = (string)wp_cache_get( $cache_key, 'gutengeek' );

	if ( !$template ) {
		$template = gutengeek_locate_template( $template_name, $template_path, $default_path );
		wp_cache_set( $cache_key, $template, 'gutengeek' );
	}
 ...
 * @return string
 */
function gutengeek_get_template_html( $template_name, $args = [], $template_path = '', $default_path = '' ) {
	ob_start();
	gutengeek_get_template( $template_name, $args, $template_path, $default_path );
	return ob_get_clean();
}
 ...
 * @return string
 */
function gutengeek_locate_template( $template_name, $template_path = '', $default_path = '' ) {
	if ( !$template_path ) {
		$template_path = gutengeek()->template_path();
	}

	if ( !$default_path ) {
		$default_path = gutengeek()->plugin_path() . '/views/templates/';
	}
