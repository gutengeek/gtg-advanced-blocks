...
class CustomFontsTable extends \WP_List_Table {
    ...
    public function column_example($item) {
    	$item_data = gutengeek_get_font_data($item['font_id']);
	...
	public function get_items($current_page = 1, $per_page = 10, $search = '', $status = '', $orderBy = 'font_id', $order = 'DESC') {
		...
		$query = "SELECT SQL_CALC_FOUND_ROWS font_id, font_name, font_user_id, font_status FROM {$wpdb->prefix}gutengeek_block_fonts WHERE 1 = 1 {$where} {$order}";
	...
	private function get_total_count() {
		global $wpdb;
		return $wpdb->get_var( "SELECT COUNT(font_id) FROM {$wpdb->prefix}gutengeek_block_fonts" );
	}
	...
	private function get_total_published_count() {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare("SELECT COUNT(font_id) FROM {$wpdb->prefix}gutengeek_block_fonts WHERE font_status = %s", 'publish') );
	}
	...
	private function get_total_trashed_count() {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare("SELECT COUNT(font_id) FROM {$wpdb->prefix}gutengeek_block_fonts WHERE font_status = %s", 'trash') );
	}
